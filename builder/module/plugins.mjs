  ...
  setup(build) {
    build.onStart(async () => {
      try {
        ...
        console.log(
          `----------- [${timeStartBuild}] НОВЫЙ БИЛД [${nameBuild}]----------`,
        );
        console.log(
          Colors.red +
            `- Очистка папки: ${build.initialOptions.outdir}` +
            Colors.reset,
        );
...
let DoneBuild = {
  ...
  setup(build) {
    build.onEnd((result) => {
      ...
      console.log(
        Colors.magenta +
          `Сборка завершилась с ${result.errors.length} ошибками` +
          Colors.reset,
      );
    });
  },
};
export { NewBuild, DoneBuild };
